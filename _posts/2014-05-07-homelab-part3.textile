...
In the "last post":/blog/homelab-part3/, we configured our Wireless AP to connect to the existing network and expose a new LAN for the lab. Now it's time to start configuring our switch to get the proper time, use the new lan network address space, hand out addresses and route to the WAP gateway to the internets.
...
Since I have 3 (and soon 4) servers, I'm going to divide up the ports in a visual way; in groups of 4: top left (green) is the host network on vlan10, top right (red) is the sometimes used vlan20 network, and the bottom left/right (blue) ports in each group will be the vim/instance vlan30 network. These will either have both ports plugged in for a bond/lagg/lacp, or will be only one port plugged in if we're also using vlan20 (top right).

Now, given that image, we will be adding ports (as they are numbered on the switch) 2, 4, 6, and 8 to vlan10.
...
<pre><code>
SG300-20#show interface switchport ge2
Port : gi2
Port Mode: Access
